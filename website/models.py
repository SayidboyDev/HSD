...
class Messege(db.Model):
    ...
    send_user_id = db.Column(db.Integer, nullable=False)
    datetime = db.Column(db.String(200), default=datetime.utcnow())

    chat_id = db.Column(db.Integer, db.ForeignKey('chats.id'))

    def __str__(self):
        return f'{self.owner_id}: {self.content} - {self.user_id}'


class Chat(db.Model):
    __tablename__ = 'chats'
    id = db.Column(db.Integer, primary_key=True)
    first_member_id = db.Column(db.Integer, nullable=False)
    second_member_id = db.Column(db.Integer, nullable=False)

    messeges = db.relationship('Messege', backref='chat')
